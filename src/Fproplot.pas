...
type
  ...
  private
    ...
    procedure DrawItmHGL(E1, E2: TElevPoints);
...
uses Dcopy, Dproplot, Fmain, Fmap, Uoutput, Ugraph, itm_output;
...
begin
  ...
  // Reverse entries in LinksList if start node is the downstream node
  if StartNode = Link1.Node2 then
  begin
    Uutils.ReverseStringList(LinksList);
    GetStartNode;
  end;
end;
...
begin
  ...
  begin
    ...
    // Retrieve link water depths if not using dynamic wave routing
    LinkData.ITMDataHigh := -1;
    if (PlotPeriod >= 0) and (aLink.OutFileIndex >= 0) then
    begin
      if Uglobals.UsesITM then
      begin

        LinkData.ITMDataHigh := itm_output.GetLinkItmDepthVals(
            aLink.OutFileIndex, PlotPeriod,
      ...
      else if not Uglobals.DynWaveFlag then
        Ylink := Uoutput.GetLinkOutVal(Vlink, PlotPeriod, aLink.OutFileIndex);
    end;
...
//    end;
    ...
    begin
      ...
      else if DynWaveFlag then
        N := GetWaterFilledPoly(Points, E1, E2)
      else begin
...
begin
  ...
  Vnode := Uoutput.GetVarIndex(Uglobals.NODEDEPTH, NODES);
    ...
    begin
      ...
      if (PlotPeriod >= 0) and (Node1.OutFileIndex >= 0) then
      begin
        Y := Uoutput.GetNodeOutVal(Vnode, PlotPeriod, Node1.OutFileIndex);
      end
      else Y := 0;

      // Draw node cylinder plus ground line
      DrawNodeElev(X, Y, Z1, Offset);
    ...
    begin
      ...
      DrawNodeElev(X, Y, Z2, Offset);
    end;
end;


procedure TProfilePlotForm.DrawNodeElev(const X: Double; const Y: Double;
  const Z: Double; const Offset: Integer);
var
  X1, X2: Integer;
  Y1, Y2, Y3: Integer;
  Yinvert, Ydepth, Ycrown: Double;
begin
  X1 := Chart1.BottomAxis.CalcXPosValue(X) + Offset;
  X2 := X1 + 7;
...
begin
  ...
  begin
    Y := Y + Uoutput.GetNodeOutVal(V, PlotPeriod, aNode.OutFileIndex)
  end;
...
procedure TProfilePlotForm.DrawItmHGL(E1, E2: TElevPoints);
//-----------------------------------------------------------------------------
// Draws a pipe's Hydraulic Grade Line on the profile plot for an ITM run.
// See note about pixel elevations in GetWaterFilledPoly function above.
//-----------------------------------------------------------------------------
var
  k: Integer;
...
begin
  ...
  // Water elev pixels below pipe invert pixels
  if (E1.Y >= E1.Yinv) and (E2.Y >= E2.Yinv) then Exit;
