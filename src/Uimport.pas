...
const
  ...
  SectionWords : array[0..59] of PChar =
    ('[TITLE',                    //0
     '[OPTION',                   //1
     '[RAINGAGE',                 //2
     '[HYDROGRAPH',               //3
...
begin
  ...
  then Result := ErrMsg(ITEMS_ERR, '')
    ...
    if Ntoks > 6 then  aNode.Data[JUNCTION_ITM_AREA_INDEX]        := tOKlIST[6];
...
begin
  if Ntoks < 7 then Result := ErrMsg(ITEMS_ERR, '')
    ...
    if (aNode1 = nil) then Result := ErrMsg(NODE_ERR, TokList[1])
      ...
      if Ntoks > 9 then aLink.Data[CONDUIT_ITM_INIT_TYPE_INDEX] := TokList[9];
      if Ntoks > 10 then aLink.Data[CONDUIT_ITM_INIT_DEPTH_INDEX] := TokList[10];
...
function ReadITMoptions: Integer;
//-----------------------------------------------------------------------------
//  Reads an ITM option from a line of input.
//-----------------------------------------------------------------------------
var
  Keyword: String;
  Index: Integer;
...
begin
  ...
  if Ntoks < 2 then exit;
  Keyword := TokList[0];
  Index := Uutils.FindKeyWord(Keyword, ItmOptionLabels, 17);

  if Index = MAX_ITM_OPTIONS then
  begin
    Project.ItmOptions[Index] := FullPathName(TokList[1]);
    exit;
  end;

  if Index = -1 then Result := ErrMsg(KEYWORD_ERR, Keyword)
  else if not Uutils.GetSingle(TokList[1], X) then
    Result := ErrMsg(NUMBER_ERR, TokList[1])
  else if X < 0 then
    Result := ErrMsg(NUMBER_ERR, TokList[1])
  else if (Index <> ITM_INIT_ELEV) and (X = 0) then
    Result := ErrMsg(NUMBER_ERR, TokList[1]);

  if Result = 0 then
    Project.ItmOptions[Index] := TokList[1];
end;

function ReadITMcontrols: Integer;
//-----------------------------------------------------------------------------
//  Reads an ITM control statement from a line of input.
//-----------------------------------------------------------------------------
var
  J, K  : Integer;
  ID    : String;
...
begin
  ...
  // Check that link being controlled exists
  ID := TokList[0];
  aLink := FindLink(ID);
  if (aLink = nil) then
  begin
    Result := ErrMsg(LINK_ERR, ID);
    exit;
  end;

  // Check that inital setting is a valid number
  if not Uutils.GetSingle(TokList[1], X) then
  begin
    Result := ErrMsg(NUMBER_ERR, TokList[1]);
    exit;
  end;

  // Check that a valid control method was supplied
  if Uutils.FindKeyWord(TokList[2], ItmControlMethods, 5) < 0 then
  begin
    Result := ErrMsg(KEYWORD_ERR, TokList[2]);
    exit;
  end;

  // Find the link property index where ITM control data begins
  // (Only pumps, orifices, and weirs have controls)
  J := aLink.Ltype;
  case J of
  ...
  aLink.Data[K] := TokList[1];                     //Initial setting
  aLink.Data[K+1] := TokList[2];                   //Control method
  if nToks > 3 then aLink.Data[K+2] := TokList[3]; //Control timeseries
  if nToks > 4 then aLink.Data[K+3] := TokList[4]; //Control node
  if nToks > 5 then aLink.Data[K+4] := TokList[5]; //Control curve
end;
...
begin
  case Section of
    ...
    58:   Result := ReadITMoptions;
    59:   Result := ReadITMcontrols;
    else  Result := 0;
  end;
end;
