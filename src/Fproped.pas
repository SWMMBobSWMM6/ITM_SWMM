...
begin
  ...
  // User is editing a Pump
  else if (CurrentList = PUMP) then
  begin
    if (Index = PUMP_CURVE_INDEX) then EditCurve(PUMPCURVE, S)
    else if (Index = PUMP_ITM_TSERIES_INDEX) then EditTimeSeries(S)
    else if (Index = PUMP_ITM_CURVE_INDEX) then EditCurve(CONTROLCURVE, S)
  end

  // User is editing an Orifice
  else if (CurrentList = ORIFICE) then
  begin
   if (Index = ORIFICE_ITM_TSERIES_INDEX) then EditTimeSeries(S)
   else if (Index = ORIFICE_ITM_CURVE_INDEX) then EditCurve(CONTROLCURVE, S)
  end

  // User is editing a Weir
  else if (CurrentList = WEIR) then
  begin
   if (Index = WEIR_COEFF_CURVE_INDEX) then EditCurve(WEIRCURVE, S)
   else if (Index = WEIR_ITM_TSERIES_INDEX) then EditTimeSeries(S)
   else if (Index = WEIR_ITM_CURVE_INDEX) then EditCurve(CONTROLCURVE, S)
  end
...
begin
  ...
  PUMP:
    ...
    PumpProps[PUMP_ITM_CURVE_INDEX].List := Project.Lists[CONTROLCURVE].Text;
    end;
  OUTLET:
    OutletProps[OUTLET_QTABLE_INDEX].List := Project.Lists[RATINGCURVE].Text;
  ORIFICE:
    WeirProps[ORIFICE_ITM_CURVE_INDEX].List := Project.Lists[CONTROLCURVE].Text;
  WEIR:
    begin
    WeirProps[WEIR_COEFF_CURVE_INDEX].List := Project.Lists[WEIRCURVE].Text;
    WeirProps[WEIR_ITM_CURVE_INDEX].List := Project.Lists[CONTROLCURVE].Text;
    end;
  end;
end;
...
begin
  ...
  PUMP:     PumpProps[PUMP_ITM_TSERIES_INDEX].List := S;
  ORIFICE:  OrificeProps[ORIFICE_ITM_TSERIES_INDEX].List := S;
  WEIR:     WeirProps[WEIR_ITM_TSERIES_INDEX].List := S;
  end;
end;
