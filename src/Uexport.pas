...
procedure ExportITM(S: TStringlist);
...
var
  ...
  ItmControls: Integer;      // # ITM controls

function HasItmControl(Setting: String; CtrlType: String): Boolean;
var
  X: Single;
begin
  Result := False;
  if not Uutils.GetSingle(Setting, X) then exit;
  if (X = 1.0) and (SameText(CtrlType, 'NONE')) then exit;
  Result := True;
end;
...
begin
  ...
  with Project.Lists[JUNCTION] do
    ...
    begin
      ...
      Line := Line + Tab + Format('%-10s', [N.Data[JUNCTION_ITM_AREA_INDEX]]);
      S.Add(Line);
    end;
end;
...
begin
  ...
  with Project.Lists[OUTFALL] do
    ...
    begin
      ...
      RouteTo := N.Data[OUTFALL_ROUTETO_INDEX];
      Line := Line + Tab + Format('%-16s', [RouteTo]);
      S.Add(Line);
    end;
end;
...
begin
  ...
  with Project.Lists[CONDUIT] do
    ...
    begin
      ...
      Line := Line + Tab + Format('%-10s', [L.Data[CONDUIT_ITM_INIT_TYPE_INDEX]]);
      Line := Line + Format('  %-10s', [L.Data[CONDUIT_ITM_INIT_DEPTH_INDEX]]);
      S.Add(Line);
    end;
end;
...
begin
  ...
  with Project.Lists[PUMP] do
    ...
    begin
      ...
      S.Add(Line);
      if HasItmControl(L.Data[PUMP_ITM_SETTING_INDEX],
        L.Data[PUMP_ITM_METHOD_INDEX]) then
        Inc(ItmControls);
    end;
end;
...
begin
  ...
  with Project.Lists[ORIFICE] do
    ...
    begin
      ...
      S.Add(Line);
      if HasItmControl(L.Data[ORIFICE_ITM_SETTING_INDEX],
        L.Data[ORIFICE_ITM_METHOD_INDEX]) then
        Inc(ItmControls);
   end;
end;
...
begin
  ...
  with Project.Lists[WEIR] do
    ...
    begin
      ...
      S.Add(Line);
      if HasItmControl(L.Data[WEIR_ITM_SETTING_INDEX],
        L.Data[WEIR_ITM_METHOD_INDEX]) then
        Inc(ItmControls);
    end;
end;
...
procedure ExportItmOptions(S: TStringlist);
//-----------------------------------------------------------------------------
var
   I: Integer;
   Line: String;
...
begin
  ...
  S.Add('[ITM_OPTIONS]');
  S.Add(';;Option            ' + Tab + 'Value');
  for I := 0 to MAX_ITM_OPTIONS-1 do
  begin
    Line := Format('%-20s', [ItmOptionLabels[I]]) + Tab + Project.ItmOptions[I];
    S.Add(Line);
  end;
  DebugFileName := Project.ItmOptions[ITM_DEBUG_FILE];
  if Length(DebugFileName) > 0 then
  begin
    DebugFileName := '"' + RelativePathName(DebugFilename) + '"';
    Line := Format('%-20s', [ItmOptionLabels[MAX_ITM_OPTIONS]]) +
      Tab + DebugFileName;
    S.Add(Line);
  end;
end;
...
procedure ExportItmControls(S: TStringlist);
//-----------------------------------------------------------------------------
var
  I, J, K: Integer;
  L: TLink;
  Line: String;
begin
  if ItmControls = 0 then exit;
  S.Add('');
  S.Add('[ITM_CONTROLS]');
  Line := ';;Link          ' + Tab + 'Setting ' + Tab + 'Control Type    ' + Tab +
   'Control Series  ' + Tab +  'Control Node    ' + Tab + 'Control Curve   ';
  S.Add(Line);
  Line := ';;--------------' + Tab + '--------' + Tab + '----------------' + Tab +
    '----------------' + Tab + '----------------' + Tab + '----------------';
  S.Add(Line);
  for J := PUMP to WEIR do
  begin
    case J of
    PUMP:    K := PUMP_ITM_SETTING_INDEX;
    ORIFICE: K := ORIFICE_ITM_SETTING_INDEX;
    WEIR:    K := WEIR_ITM_SETTING_INDEX;
    else continue;
    end;
    ...
    begin
      ...
      begin
        L := TLink(Objects[I]);
        if not HasItmControl(L.Data[K], L.Data[K+1]) then continue;
        Line := Format('%-16s', [Strings[I]]);
        Line := Line + Tab + Format('%-8s', [L.Data[K]]);    //Setting
        Line := Line + Tab + Format('%-16s', [L.Data[K+1]]);  //Ctrl Type
        Line := Line + Tab + Format('%-16s', [L.Data[K+2]]); //Time Series
        Line := Line + Tab + Format('%-16s', [L.Data[K+3]]); //Ctrl Node
        Line := Line + Tab + Format('%-16s', [L.Data[K+4]]); //Ctrl Curve
        S.Add(Line);
      end;
    end;
  end;
...
var
  ...
  F4: String;
begin
  // Create names for saved output files
  Result := False;
  F1 := ChangeFileExt(Fname, '.rpt');
  F2 := ChangeFileExt(Fname, '.out');
  F3 := ChangeFileExt(Fname, '.itm');
  F4 := ChangeFileExt(Fname, '.plt');

  // Make sure that file names not same name as input file Fname
  if (SameText(F1, Fname)) or (SameText(F2, Fname)) then Exit;
  if Uglobals.UsesITM then
  begin
    if (SameText(F3, Fname)) or (SameText(F4, Fname)) then Exit;
  end;

  // Delete any previously saved output files
  Uoutput.CloseOutputFile;
  if not SameText(F1, TempReportFile) then DeleteFile(PChar(F1));
  if not SameText(F2, TempOutputFile) then DeleteFile(PChar(F2));
  if Uglobals.UsesITM then
  begin
    if not SameText(F3, TempItmOutFile) then DeleteFile(Pchar(F3));
    if not SameText(F4, TempItmPltFile) then DeleteFile(Pchar(F4));
  end;

  // Rename current temporary files to saved file names
  if not RenameFile(TempReportFile, F1) then Exit;
  if not RenameFile(TempOutputFile, F2) then Exit;
  if Uglobals.UsesITM then
  begin
    if not RenameFile(TempItmOutFile, F3) then Exit;
    if not RenameFile(TempItmPltFile, F4) then Exit;
  end;
  Result := True;
end;
