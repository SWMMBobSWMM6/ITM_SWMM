...
type
  ...
  private
    ...
    function CheckSkip(Line:String; Skip: Boolean): Boolean;
    function CheckItmSkip(Line: String; Skip:Boolean): Boolean;
  public
    { Public declarations }
      ...
      begin
        ...
        begin
          ...
          if Uglobals.UsesITM then
            Skip := CheckItmSkip(Line, Skip)
          else
            Skip := CheckSkip(Line, Skip);
...
function TStatusForm.CheckItmSkip(Line: String; Skip: Boolean): Boolean;
begin
  Result := Skip;
  if Skip = False then
  begin
//    if ContainsText(Line, 'Flow Routing Method') then Result := True
//    else if ContainsText(Line, '***') then Result := True
    if ContainsText(Line, 'Subcatchment Runoff') then
    begin
     with Memo1 do
       Lines.Delete(Lines.Count-1);
     Result := True
    end
    else if ContainsText(Line, 'Node Depth') then Result := True;
  end
  else if Skip = True then
  begin
//    if ContainsText(Line, ' Starting Date') then Result := False
    if ContainsText(Line, 'ILLINOIS TRANSIENT MODEL') then Result := False
    else if ContainsText(Line, 'Total elapsed') then Result := False;
  end;
end;

function TStatusForm.CheckSkip(Line:String; Skip: Boolean): Boolean;
begin
  Result := Skip;
  if Skip = False then
  begin
    if  (ContainsText(Line, 'Subcatchment Runoff'))
    or (ContainsText(Line, 'Node Depth')) then Result := True;
  end
  else if ContainsText(Line, 'Analysis begun') then Result := False;
end;
