...
uses
  ...
  Buttons, ImgList, Grids, FileCtrl, Uproject, Uglobals, Uutils, UpDnEdit;
...
type
  TAnalysisOptionsForm = class(TForm)
    ...
    InitElevLabel: TLabel;
    WaveCelerityEdit: TNumEdit;
    WaveCelerityLabel: TLabel;
    Label34: TLabel;
    Label35: TLabel;
    Label36: TLabel;
    MaxCellsEdit: TEdit;
    MaxCellsUpDn: TUpDown;
    MaxPlotCellsEdit: TEdit;
    MaxPlotCellsUpDn: TUpDown;
    RefDepthUpDn: TUpDown;
    RefDepthEdit: TEdit;
    MaxTimeStepEdit: TNumEdit;
    Label38: TLabel;
    ItmDefaultsLinkLabel: TLinkLabel;
    Label37: TLabel;
    DebugFileBtn: TBitBtn;
    DebugFileCB: TCheckBox;
    DebugFileEdit: TEdit;
    ...
    procedure ItmDefaultsLinkLabelLinkClick(Sender: TObject; const Link: string;
      LinkType: TSysLinkType);
    procedure DebugFileCBClick(Sender: TObject);
    procedure DebugFileBtnClick(Sender: TObject);
  ...
  private
    ...
    procedure SetItmOptions(Options: array of String);
    procedure GetItmOptions(var Options: array of String);
...
begin
  ...
  begin
    ...
    WaveCelerityLabel.Caption := 'Pressure Wave Celerity (ft/s)';
    InitElevLabel.Caption := 'Global Initial Water Elevation (ft)';
  end
  else
  begin
    MinSurfAreaLabel.Caption := TXT_MIN_SURF_AREA + TXT_SQUARE_METERS;
    HeadTolLabel.Caption := TXT_HEAD_TOL + TXT_TOL_METERS;
    WaveCelerityLabel.Caption := 'Pressure Wave Celerity (m/s)';
    InitElevLabel.Caption := 'Global Initial Water Elevation (m)';
end;
  ...
  with MainForm.ProjectImageList do
    GetBitmap(GetIndexByName('browse'), DebugFileBtn.Glyph);
...
begin
  ...
  4: ActiveControl := MaxTimeStepEdit;
  5: ActiveControl := AddBtn;
  end;
  HasChanged := False;
end;
...
begin
  ...
  SetItmOptions(Project.ItmOptions);
...
begin
  ...
  GetItmOptions(Project.ItmOptions);
...
procedure TAnalysisOptionsForm.DebugFileBtnClick(Sender: TObject);
begin
  with MainForm.SaveDialog do
  begin
    Title := 'ITM Debug File';
    Filter := 'ITM Debug Files (*.txt)|*.txt|All files|*.*';
    InitialDir := ProjectDir;
    DefaultExt := 'txt';
    if Length(DebugFileEdit.Text) > 0
    then Filename := ChangeFileExt(ExtractFileName(DebugFileName),'.txt')
    else Filename := '*.txt';
    if Execute then
    begin
      DebugFileName := Filename;
      DebugFileEdit.Text :=
        MinimizeName(DebugFileName, self.Canvas, DebugFileEdit.ClientWidth);
    end;
  end;

end;

procedure TAnalysisOptionsForm.DebugFileCBClick(Sender: TObject);
begin
  if not DebugFileCB.Checked then
  begin
   DebugFileName := '';
   DebugFileEdit.Text := '';
  end;
  DebugFileEdit.Enabled := DebugFileCB.Checked;
  DebugFileBtn.Visible := DebugFileCB.Checked;
end;
...
procedure TAnalysisOptionsForm.SetItmOptions(Options: array of String);
begin
  MaxTimeStepEdit.Text   := Options[ITM_MAX_STEP];
  MaxCellsUpDn.Position  := StrToIntDef(Options[ITM_MAX_CELLS],
                            StrToInt(DefItmOptions[ITM_MAX_CELLS]));
  MaxPlotCellsUpDn.Position := StrToIntDef(Options[ITM_MAX_PLOT_CELLS],
                               StrToInt(DefItmOptions[ITM_MAX_PLOT_CELLS]));
  RefDepthUpDn.Position  := StrToIntDef(Options[ITM_REF_DEPTH],
                            StrToInt(DefItmOptions[ITM_REF_DEPTH]));
  WaveCelerityEdit.Text  := Options[ITM_WAVE_CELERITY];
  InitElevEdit.Text      := Options[ITM_INIT_ELEV];
  DebugFileName          := Options[ITM_DEBUG_FILE];
  DebugFileEdit.Text     :=
    MinimizeName(DebugFileName, self.Canvas, DebugFileEdit.ClientWidth);
  DebugFileCB.Checked    := Length(DebugFileName) > 0;
  DebugFileEdit.Enabled  := DebugFileCB.Checked;
  DebugFileBtn.Visible   := DebugFileCB.Checked;
end;

procedure TAnalysisOptionsForm.GetItmOptions(var Options: array of String);
begin
  Options[ITM_MAX_STEP]       := MaxTimeStepEdit.Text;
  Options[ITM_MAX_CELLS]      := IntToStr(MaxCellsUpDn.Position);
  Options[ITM_MAX_PLOT_CELLS] := IntToStr(MaxPlotCellsUpDn.Position);
  Options[ITM_REF_DEPTH]      := IntToStr(RefDepthUpDn.Position);
  Options[ITM_WAVE_CELERITY]  := WaveCelerityEdit.Text;
  Options[ITM_INIT_ELEV]      := InitElevEdit.Text;
  Options[ITM_DEBUG_FILE]     := DebugFileName;
end;

procedure TAnalysisOptionsForm.ItmDefaultsLinkLabelLinkClick(Sender: TObject;
  const Link: string; LinkType: TSysLinkType);
begin
  SetItmOptions(DefItmOptions);
end;
