...
uses
  Fmain, Ubrowser, Dreporting, Uresults, itm_output;
...
begin
  ...
  try
    ...
    // Check if ITM simulation was successful
    else if Uglobals.UsesITM
    and SameText(Project.Options.Data[IGNORE_ROUTING_INDEX], 'NO')
    then
      Result := itm_output.CheckItmRunStatus
  ...
  begin
   FileClose(Fout);
   if Uglobals.UsesITM then itm_output.CloseItmOutputFile;
...
begin
  ...
  begin
    Result := itm_output.GetLinkItmOutVal(V, Period, Zindex);
    exit;
  end;
...
begin
  if Uglobals.UsesITM then
  begin
    itm_output.GetLinkItmOutVals(LinkVar, Period, Value);
    exit;
  end;
...
begin
  ...
  if Uglobals.UsesITM then
  begin
    Result := itm_output.GetNodeItmOutVal(V, Period, Zindex);
    exit;
  end;
...
begin
  if Uglobals.UsesITM then
  begin
    itm_output.GetNodeItmOutVals(NodeVar, Period, Value);
    exit;
  end;
...
begin
  ...
  F4 := '';
  ItmUsed :=
    SameText(Project.Options.Data[ROUTING_MODEL_INDEX], 'ITM') and
    SameText(Project.Options.Data[IGNORE_ROUTING_INDEX], 'NO');
  if ItmUsed then
  begin
  ...
  begin
    ...
    if not FileExists(F3) or not FileExists(F4) then Exit;
  end;

  // Rename these files as the temporary output files used to store results
  TempReportFile := F1;
  TempOutputFile := F2;
  TempItmOutFile := F3;
  TempItmPltFile := F4;
...
var
  P: Int64;
begin
  if Uglobals.UsesITM and
    (V in [SYS_DWFLOW, SYS_EXFLOW, SYS_INFLOW, SYS_FLOODING,
          SYS_OUTFLOW, SYS_STORAGE]) then
  begin
    Result := MISSING;
    exit;
  end;
...
begin
  ...
  begin
    ...
    else if Uutils.GetSingle(L.Data[K], X) then
    begin
      if X = MISSING then L.ColorIndex := -1
      else L.ColorIndex := FindLinkColor(Abs(X));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      L := Project.GetLink(I, J);
      K := L.RptFileIndex;
      if (K >= 0) and (Zlink[K] <> MISSING) then
...
begin
  ...
  begin
    ...
    begin
      N:= Project.GetNode(I, J);
      K := N.RptFileIndex;
      if (K >= 0) and (Znode[K] <> MISSING) then
