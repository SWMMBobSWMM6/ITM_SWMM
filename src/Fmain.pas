...
//  MSG_NOT_EPASWMM_FILE = 'Not a SWMM file.';
  ...
  TXT_SAVE_ITM_AS  = 'Save ITM Interface File As';
  TXT_ITM_FILTER = 'ITM files (*.itm)|*.itm|All files|*.*';
...
type
  TMainForm = class(TForm)
    ...
    procedure MnuExportITMClick(Sender: TObject);
...
uses
  ...
  Utools, Uupdate, Dreporting, Dproselect, Dtimeplot, itm_export;
...
// (Uglobals.Startup is called from itmswmm.dpr before the main form is
...
begin
  ...
  MnuExportITM.Visible := SameText(Project.Options.Data[ROUTING_MODEL_INDEX], 'ITM');
end;
...
procedure TMainForm.MnuExportITMClick(Sender: TObject);
var
  Fname: String;
begin
  with SaveDialog do
  begin
    Title := TXT_SAVE_ITM_AS;
    Filter := TXT_ITM_FILTER;
    DefaultExt := TXT_ITM;
    Filename := '*.' + TXT_ITM;
    InitialDir := ProjectDir;
    if Execute then Fname := Filename;
  ...
  Application.ProcessMessages;
  if Length(Fname) > 0 then itm_export.ExportItmProject(Fname, '');
end;
...
begin
  ...
  begin
    FileName := Uutils.GetTempFile(Uglobals.TempDir, 'itm');

    Uglobals.TempItmInpFile := ChangeFileExt(FileName, '.inp');
    Uglobals.TempItmRptFile := ChangeFileExt(FileName, '.rpt');
    Uglobals.TempItmLogFile := ChangeFileExt(FileName, '.log');
    Uglobals.TempItmOutFile := ChangeFileExt(FileName, '.out');
    Uglobals.TempItmPltFile := ChangeFileExt(FileName, '.plt');
    SysUtils.DeleteFile(FileName);
  end
  else begin
    Uglobals.TempItmInpFile := '';
    Uglobals.TempItmRptFile := '';
    Uglobals.TempItmLogFile := '';
    Uglobals.TempItmOutFile := '';
    Uglobals.TempItmPltFile := '';
  end;
...
begin
  ...
  SysUtils.DeleteFile(Uglobals.TempItmInpFile);
  SysUtils.DeleteFile(Uglobals.TempItmRptFile);
  SysUtils.DeleteFile(Uglobals.TempItmLogFile);
  if not Uglobals.ResultsSaved then
  begin
    SysUtils.DeleteFile(Uglobals.TempReportFile);
    SysUtils.DeleteFile(Uglobals.TempOutputFile);
    SysUtils.DeleteFile(Uglobals.TempItmOutFile);
    SysUtils.DeleteFile(Uglobals.TempItmPltFile);
   end;
end;
...
begin
  ...
  Uglobals.ResultsSaved := False;
  Uglobals.UsesITM := SameText(Project.Options.Data[ROUTING_MODEL_INDEX], 'ITM');
  DeleteTempFiles;
  CreateTempFiles;
...
begin
  HtmlHelp(GetDesktopWindow, ItmSwmmDir + INLETSTUTORFILE, HH_DISPLAY_TOC, 0);
end;

procedure TMainForm.MnuIntroTutorialClick(Sender: TObject);
begin
  HtmlHelp(GetDesktopWindow, ItmSwmmDir + BASICTUTORFILE, HH_DISPLAY_TOC, 0);
end;
...
procedure TMainForm.MnuItmHelpClick(Sender: TObject);
begin
  HtmlHelp(GetDesktopWindow, ItmSwmmDir + ITMHELPFILE, HH_DISPLAY_TOC, 0);
end;
